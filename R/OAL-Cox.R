...
#' @return RASCE of OAL-Cox model, the optimal value of the parameter λ and propensity score coefficients corresponding to this value
...
OAL_Cox <- function(data, A, time, status, covariates, lambda_vec, gamma_convergence_factor, tau){
  ...
  ### Want to save RASCE, wAMD and propensity score coefficients for each lambda value
  RASCE = wAMD_vec <- rep(NA, length(lambda_vec))
  names(RASCE) = names(wAMD_vec) <- names(lambda_vec)
  ...
  # print out RASCE corresponding to smallest wAMD value
