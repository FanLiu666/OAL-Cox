...
simulate_data <- function(mean_x, sig_x, rho, n, p, lambda, alpha, theta){
  ...
  # set.seed(20240510)
  # simulate failure time T_F
  u <- runif(n, 0, 1)
  epsilon <- rnorm(n = n, sd = sig_x)
  gT_Xβ <- rowSums(Data[ , var.list]*matrix(beta_v, nrow = n, ncol = length(var.list), byrow = TRUE))
  gT_Xβ <- gT_Xβ + epsilon
  gT_XA <- gT_Xβ - Data$A*bA
  Data$Sur <- (diag(-log(u)/lambda) %*% exp(-gT_XA))^(1/alpha)

  # simulate censoring time T_C
  CT <- runif(n, 0, theta)
  ...
  Data <- Data %>%
    select(-Sur)
  # censor_freq <- sum(Data$status == 0)/n
